...
(function() {
    ...
    function multiLines2D(name, options, scene) {
        var system = [];
        var paths = options.paths;
        for (var i = 0; i < paths.length; i++) {
            var path = line2D(name, { path: paths[i], width: options.width }, scene);
            // var path = Arminia.line2D(name, { path: paths[i], width: options.width }, scene);
            system.push(path);
        }
        return BABYLON.Mesh.MergeMeshes(system, true);
    }
    ...
    Arminia.multiLines2D = multiLines2D;
